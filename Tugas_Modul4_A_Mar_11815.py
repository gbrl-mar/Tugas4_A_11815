...
def scatter(model, model_name, data, new_point, features, color_scale, title):
    ...
    if model_name == "KMeans_model":
        new_cluster = model.predict(new_point[features])[
            0
        ]  
    else:
        distances = pairwise_distances(
            new_point[features], data[features]
...
st.set_page_config(
    page_title="11815 - Unsupervised Learning", 
    page_icon="",
    layout="wide",
...
if uploaded_file is not None:
    ...
    st.markdown(
        "<h1 style='text-align: center;'>Unsupervised Learning - Gabriel Mario Binsar</h1>",
        unsafe_allow_html=True,
    )
    st.dataframe(input_data)

    # Direktori tempat penyimpanan ketiga model yang telah di dump sebelumnya
    model_directory = r"C:\PBP\Unsupervised Learning (Praktek)\Tugas4_X_YYYYY"
    model_path = {
    ...
    # Memuat model jika file ada di path
    ...
    # Input data untuk prediksi titik baru
    ...
    if st.sidebar.button("Prediksi !"):
        ...
        for i, (model_name, model, title, color_scale) in enumerate(cluster_method):
            fig, new_cluster = scatter(
                model, model_name, input_data, new_point, features, color_scale, title
            )
            with cols[i]:
                st.plotly_chart(fig)
            st.markdown(
                f"<p style='text-align: center;'>Titik dari data yang baru masuk ke dalam cluster: {new_cluster}</p>",
                unsafe_allow_html=True,
            )
